#!/bin/bash
#SBATCH --job-name=sae_concept_fine_tuning
#SBATCH --output=sbatch_output/%j_fine_tuning.out
#SBATCH --error=sbatch_output/%j_fine_tuning.err
...
# SCRIPT_NAME="/leonardo/home/userexternal/ecassano/projects/SAeUron_finetuning/scripts/sae_finetuning.py"
...
OBJECT_SCORES_DIR="/leonardo_work/IscrC_MAGNIFY/cassano/saeuron/scores/objects/non_finetuned/scores.json"
STYLE_SCORES_DIR="/leonardo_work/IscrC_MAGNIFY/cassano/saeuron/scores/styles/non_finetuned/scores.json"

# Choose which type of fine-tuning to run
CONCEPT_TYPE="object" 

# Directory to save models and logs
SAVE_DIR="/leonardo_work/IscrC_MAGNIFY/cassano/saeuron/sae_checkpoints/object_concept_optimized/v4_before_topk/ce_weight_3.0_sparsity_0.01"
...
# Run training with corrected environment variables
...
torchrun --nproc_per_node=4 ${SCRIPT_NAME} \
    ...
    --object_scores_json_path ${OBJECT_SCORES_DIR} \
    --style_scores_json_path ${STYLE_SCORES_DIR} \
    --device cuda \
    --learning_rate 1e-6 \
    --num_epochs 50 \
