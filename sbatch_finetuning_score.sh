#!/bin/bash
#SBATCH --job-name=score_fine_tuning
#SBATCH --output=sbatch_output/%j_score_fine_tuning.out
#SBATCH --error=sbatch_output/%j_score_fine_tuning.err
...
# Name of the NEW dual concept Python script with score loss
SCRIPT_NAME="/leonardo/home/userexternal/ecassano/projects/SAeUron_finetuning/scripts/sae_finetuning_score.py"
...
# Directory to save models and logs - UPDATED for score loss
SAVE_DIR="/leonardo_work/IscrC_MAGNIFY/cassano/saeuron/sae_checkpoints/score_loss_optimized/batch_topk/"
...
# Run training with the NEW score loss script
echo "Running dual object-style concept training with SCORE LOSS..."
...
if [ $? -eq 0 ]; then
    echo "✅ Score loss training completed successfully!"
else
    echo "❌ Score loss training failed with exit code: $?"
fi
...
echo ""
echo "=== SCORE LOSS TRAINING SUMMARY ==="
...
echo "Training Type: Dual Object-Style Concept Assignment with SCORE LOSS"
echo "Score Weight: ${score_weight}"
echo "Loss Function: MSE Reconstruction + Score Loss"
echo "=================================="