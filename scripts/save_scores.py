...
# Predefined y-limits for each concept based on your provided data
CONCEPT_Y_LIMITS = {
    'Architectures': {'max': 0.0229, 'min': -0.0064},
    'Bears': {'max': 0.0312, 'min': -0.0064},
    'Birds': {'max': 0.0270, 'min': -0.0065},
    'Butterfly': {'max': 0.0205, 'min': -0.0045},
    'Cats': {'max': 0.0279, 'min': -0.0050},
    'Dogs': {'max': 0.0203, 'min': -0.0044},
    'Fishes': {'max': 0.0246, 'min': -0.0049},
    'Flame': {'max': 0.0352, 'min': -0.0050},
    'Flowers': {'max': 0.0404, 'min': -0.0045},
    'Frogs': {'max': 0.0277, 'min': -0.0051},
    'Horses': {'max': 0.0191, 'min': -0.0061},
    'Human': {'max': 0.0097, 'min': -0.0067},
    'Jellyfish': {'max': 0.0487, 'min': -0.0068},
    'Rabbits': {'max': 0.0283, 'min': -0.0061},
    'Sandwiches': {'max': 0.0299, 'min': -0.0068},
    'Sea': {'max': 0.0339, 'min': -0.0071},
    'Statues': {'max': 0.0295, 'min': -0.0067},
    'Towers': {'max': 0.0214, 'min': -0.0076},
    'Trees': {'max': 0.0446, 'min': -0.0061},
    'Waterfalls': {'max': 0.0455, 'min': -0.0071}
}
...
def plot_concept_scores(all_scores, output_dir=None, save_format='pdf', plot_style='line', 
                        marker_size=6, line_width=2.5, use_predefined_limits=True, 
                        padding_factor=0.05):
    """
    ...
    Uses predefined y-limits for consistent scaling across concepts.
    ...
    for concept in concepts:
        ...
        if plot_style == 'line':
            ax.plot(neuron_indices, avg_scores, linewidth=line_width, color="#2ca02c")
        elif plot_style == 'scatter':
            ax.scatter(neuron_indices, avg_scores, s=marker_size, alpha=0.7, color="#2ca02c")
        elif plot_style == 'both':
            ax.plot(neuron_indices, avg_scores, linewidth=line_width, alpha=0.8, color="#2ca02c")
            ax.scatter(neuron_indices, avg_scores, s=marker_size, alpha=0.9, color="#2ca02c")
...
def plot_timestep_histograms(all_scores, concept, output_dir=None, save_format='pdf', bins=50, alpha=0.8):
    ...
    Args:
        all_scores (dict): Dictionary with concept names as keys and lists of score arrays as values
        concept (str): Specific concept to plot
    ...
    # Set title (no LaTeX escaping needed)
    fig.suptitle(f'Score Distributions Across Timesteps for {concept}', 
...
def main(args):
    ...
    if args.plot_scores:
        ...
        if args.plot_timesteps_concept:
            print(f"Generating timestep histograms for {args.plot_timesteps_concept}...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_predefined_limits", type=bool, default=True,
