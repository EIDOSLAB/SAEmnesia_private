...
from balanced_concept_dataloader import MetadataBalancedDataset
...
def load_datasets_from_category_dirs_with_styles(base_dirs, hookpoint, dtype=torch.float32):
    ...
    FIXED VERSION: Actually uses the recovered style metadata instead of overwriting it.
    """
    datasets = []
        ...
        for concept_dir in concept_subdirs:
            ...
            if (concept_dir / "dataset_info.json").exists():
                ...
                if concept_name not in object_style_index:
                    print(f"    ⚠️  No style recovery data for '{concept_name}', keeping original labels")
                    # Keep whatever labels exist, just ensure object_label is correct
                    ...
                    # Don't touch style_label - keep whatever was there
                    datasets.append(dataset)
                    continue
                
                # FIXED: Actually use the recovered metadata to create proper style labels
                style_datasets = []
                ...
                for style_name, style_entries in object_style_index[concept_name].items():
                    for entry in style_entries:
                        ...
                        try:
                            ...
                            # FIXED: Remove existing labels and add CORRECT ones from metadata
                            ...
                            # Add correct labels from the recovered metadata
...
class SAEConceptLatentOptimizer:
    ...
    def create_balanced_loaders_after_assignment(self, hook_name):
        """Create balanced data loaders using metadata after concept assignments are made."""
        print(f"\n{'='*60}")
        print(f"CREATING METADATA-BASED BALANCED DATA LOADERS FOR {hook_name}")
        print(f"{'='*60}")

        object_to_latent = self.object_to_latent[hook_name]
        style_to_latent = self.style_to_latent[hook_name]

        if not object_to_latent and not style_to_latent:
            print("No concept mappings found, keeping standard loaders")
            return
        ...
        print(f"Creating metadata-based balanced training dataset...")

        # Use MetadataBalancedDataset with your activations directory and hookpoint
        balanced_train_dataset = MetadataBalancedDataset(
            self.train_dataset, 
            object_to_latent, 
            style_to_latent,
            self.batch_size, 
            balance_ratio=0.5, 
            seed=self.seed,
            activations_dir=str(self.activations_dir),  # Pass your activations directory
            hookpoint=hook_name  # Pass the hookpoint name
        )
        ...
        self.train_loader = DataLoader(
            ...
            shuffle=False,  # Shuffling handled by balanced dataset
            num_workers=2,
            pin_memory=True,
            collate_fn=dual_label_collate_fn,
            drop_last=True
        )

        # Validation loader remains standard (no need to balance validation)
        ...
        print(f"✅ Created metadata-based balanced train loader: {len(self.train_loader)} batches")
        print(f"✅ Created standard val loader: {len(self.val_loader)} batches")
        print(f"{'='*60}\n")

    # Also update your import at the top of the file:
    # from balanced_concept_dataloader import MetadataBalancedDataset
    ...
    def _create_dual_data_loaders(self, dataset_dict):
        """DataLoader creation with balanced concept sampling."""
        ...
        # NOTE: We'll create the actual balanced loaders after concept assignment
        # For now, create temporary standard loaders
        def dual_label_collate_fn(batch):
            """Collate function that handles both object and style labels."""
            activations = torch.stack([item['activations'] for item in batch])
            object_labels = [item['object_label'] for item in batch]
        ...
        # Temporary loaders - will be replaced after concept assignment
        self.train_loader = DataLoader(
    ...
    def print_epoch_summary(self, epoch, hook_name, train_losses, val_losses, 
        ...
        ce_diff = val_losses['score_loss'] - train_losses['score_loss']
        ...
        print(f"{'Cross Entropy':<20} {train_losses['score_loss']:<12.6f} {val_losses['score_loss']:<12.6f} {ce_diff:>+12.6f}")
    ...
    def initialize_wandb(self):
        """Initialize weights and biases for logging with enhanced configuration for run comparisons."""
        if WANDB_AVAILABLE:
            ...
            model_info = f"{len(self.saes)}layers" if self.saes else "0layers"
            training_type = "scratch" if self.from_scratch else ("resume" if self.resume else "finetune")
            
            run_name = (f"{training_type}_{model_info}_"
                       f"lr{self.lr}_bs{self.batch_size}_"
                       f"rw{self.reconstruction_weight}_sw{self.score_weight}_"
                       f"{timestamp}")
            
            # Add meaningful tags for filtering and grouping
            tags = [
                training_type,
                f"epochs_{self.num_epochs}",
            ...
            if hasattr(self, 'object_to_latent') and hasattr(self, 'style_to_latent'):
                if len(self.object_to_latent) > 0 and len(self.style_to_latent) > 0:
                    tags.append("dual_concept")
                elif len(self.object_to_latent) > 0:
                    tags.append("object_only")
                elif len(self.style_to_latent) > 0:
                    tags.append("style_only")
            ...
            if self.mixed_batches:
                tags.append("mixed_batches")
            
            # Comprehensive configuration for comparisons
            config = {
                ...
                # Loss weights (critical for comparisons)
                "reconstruction_weight": self.reconstruction_weight,
                "score_weight": self.score_weight,
                
                # Model architecture info
                "num_layers": len(self.saes) if self.saes else 0,
                "layer_names": list(self.saes.keys()) if self.saes else [],
                
                # Data configuration
                "validation_split": self.validation_split,
                "mixed_batches": self.mixed_batches,
                "activation_column": self.activation_column,
                
                # Training mode and resume info
            ...
            # Add concept assignment info if available
            try:
                if hasattr(self, 'object_to_latent') and self.saes:
                    first_hook = list(self.saes.keys())[0] if self.saes else None
                    if first_hook:
                        config.update({
                            "num_object_concepts": len(self.object_to_latent.get(first_hook, {})),
                            "num_style_concepts": len(self.style_to_latent.get(first_hook, {})),
                            "total_concepts": len(self.object_to_latent.get(first_hook, {})) + len(self.style_to_latent.get(first_hook, {})),
                        })
            except Exception as e:
                print(f"Warning: Could not add concept info to config: {e}")
            
            # Add SAE-specific configuration if available
            try:
                if self.saes:
                    first_sae = next(iter(self.saes.values()))
                    model = first_sae.module if hasattr(first_sae, 'module') else first_sae
                    config.update({
                        "sae_d_in": getattr(model, 'd_in', None),
                        "sae_num_latents": getattr(model, 'num_latents', None),
                    })
                    
                    # Add SAE config parameters if available
                    if hasattr(model, 'cfg') and model.cfg:
                        sae_cfg = model.cfg
                        config.update({
                            "sae_expansion_factor": getattr(sae_cfg, 'expansion_factor', None),
                            "sae_k": getattr(sae_cfg, 'k', None),
                            "sae_normalize_decoder": getattr(sae_cfg, 'normalize_decoder', None),
                            "sae_batch_topk": getattr(sae_cfg, 'batch_topk', None),
                        })
            except Exception as e:
                print(f"Warning: Could not add SAE config info: {e}")
            ...
            wandb.init(
                ...
                name=run_name,
                tags=tags,
                group=f"experiment_{datetime.datetime.now().strftime('%Y%m%d')}", # Group runs by day
                config=config,
                dir=wandb_dir
            )
            
            # Set up custom metrics and charts for better visualization
    ...
    def log_summary_metrics(self, epoch, hook_name, train_losses, val_losses, 
                           train_success_rate, val_success_rate):
        """Log high-level summary metrics for easy comparison across runs."""
        if not WANDB_AVAILABLE or self.rank != 0:
            return
            
        try:
            # Calculate derived metrics for comparison
            overfitting_ratio = val_losses['total_loss'] / max(train_losses['total_loss'], 1e-8)
            concept_generalization = val_success_rate / max(train_success_rate, 0.01)  # Avoid div by 0
            
            # Loss component analysis
            recon_dominance = train_losses['recon_loss'] / max(train_losses['total_loss'], 1e-8)
            score_dominance = train_losses['score_loss'] / max(train_losses['total_loss'], 1e-8)
            
            # Training efficiency metrics
            improvement_rate = (self.best_val_loss - val_losses['total_loss']) / max(self.best_val_loss, 1e-8)
            
            summary_metrics = {
                f"{hook_name}/summary/overfitting_ratio": overfitting_ratio,
                f"{hook_name}/summary/concept_generalization": concept_generalization,
                f"{hook_name}/summary/recon_loss_ratio": recon_dominance,
                f"{hook_name}/summary/score_loss_ratio": score_dominance,
                f"{hook_name}/summary/improvement_rate": improvement_rate,
                f"{hook_name}/summary/total_concepts": len(self.object_to_latent.get(hook_name, {})) + len(self.style_to_latent.get(hook_name, {})),
                f"{hook_name}/summary/epochs_completed": epoch,
            }
            
            wandb.log(summary_metrics)
            
        except Exception as e:
            print(f"Warning: Could not log summary metrics: {e}")
    
    def log_final_summary(self):
        """Log final metrics for hyperparameter optimization and run comparison."""
        if not WANDB_AVAILABLE or self.rank != 0:
            return
        ...
        try:
            final_metrics = {
                ...
                "final/completed_epochs": max(1, self.start_epoch - 1),
                "final/early_stopped": self.patience_counter >= self.patience,
                "final/training_efficiency": self.best_val_loss / max(self.num_epochs, 1),
            }
            
            # Log as both regular metrics and summary for easy access
            wandb.log(final_metrics)
            for key, value in final_metrics.items():
                wandb.run.summary[key] = value
                
            # Add configuration summary to run summary for easy filtering
            wandb.run.summary.update({
                "config/learning_rate": self.lr,
                "config/reconstruction_weight": self.reconstruction_weight,
                "config/score_weight": self.score_weight,
                "config/from_scratch": self.from_scratch,
            })
            
        except Exception as e:
            print(f"Warning: Could not log final summary: {e}")
    def _setup_wandb_charts(self):
        """Set up custom wandb charts for better comparison and visualization."""
        if not WANDB_AVAILABLE or self.rank != 0:
            return
        ...
        try:
            ...
            # Loss metrics with epoch as x-axis
            wandb.define_metric("*/train/total_loss", step_metric="epoch")
            wandb.define_metric("*/train/recon_loss", step_metric="epoch")
            wandb.define_metric("*/train/score_loss", step_metric="epoch")
            wandb.define_metric("*/val/total_loss", step_metric="epoch")
            wandb.define_metric("*/val/recon_loss", step_metric="epoch")
            wandb.define_metric("*/val/score_loss", step_metric="epoch")

            # Success rate metrics
            wandb.define_metric("*/train/concept_success_rate", step_metric="epoch")
            wandb.define_metric("*/val/concept_success_rate", step_metric="epoch")

            # Summary metrics for easy comparison
            wandb.define_metric("*/summary/overfitting_ratio", step_metric="epoch")
            wandb.define_metric("*/summary/concept_generalization", step_metric="epoch")
            wandb.define_metric("*/summary/recon_loss_ratio", step_metric="epoch")
            wandb.define_metric("*/summary/score_loss_ratio", step_metric="epoch")

            # Early stopping metrics
            wandb.define_metric("*/best_val_loss", step_metric="epoch")
            wandb.define_metric("*/patience_counter", step_metric="epoch")

        except Exception as e:
            print(f"Warning: Could not set up wandb charts: {e}")
    ...
    def compute_score_loss(self, pre_acts, object_labels, style_labels, object_to_latent, style_to_latent, delta=1e-8, **kwargs):
        """
        FIXED score loss computation with comprehensive bounds checking and error handling.
        """
        # Input validation
        if pre_acts is None or len(object_labels) == 0 or len(style_labels) == 0:
            print(f"  Score Loss: Invalid inputs - skipping")
            return torch.tensor(0.0, device=self.device, dtype=self.dtype)
        
        # Handle different input shapes with strict validation
        if len(pre_acts.shape) == 2:
            batch_times_seq, num_latents = pre_acts.shape
            batch_size = len(object_labels)
    
            # CRITICAL: Validate dimensions before reshaping
            if batch_times_seq != batch_size:
                if batch_times_seq % batch_size == 0:
                    ...
                    if seq_length > 0:
                        try:
                            ...
                            print(f"  Score Loss: Reshaped from ({batch_times_seq}, {num_latents}) to ({batch_size}, {num_latents})")
                        except Exception as e:
                            print(f"  Score Loss Error: Reshape failed - {e}")
                            return torch.tensor(0.0, device=self.device, dtype=self.dtype)
                    else:
                        print(f"  Score Loss Error: Invalid sequence length {seq_length}")
                        return torch.tensor(0.0, device=self.device, dtype=self.dtype)
                else:
                    print(f"  Score Loss Error: Cannot reshape {batch_times_seq} to match batch size {batch_size}")
                    return torch.tensor(0.0, device=self.device, dtype=self.dtype)
    
        elif len(pre_acts.shape) == 3:
            batch_size, seq_len, num_latents = pre_acts.shape
            if batch_size != len(object_labels):
                print(f"  Score Loss Error: 3D shape mismatch - {batch_size} vs {len(object_labels)}")
                return torch.tensor(0.0, device=self.device, dtype=self.dtype)
            pre_acts = pre_acts.mean(dim=1)
            
        else:
            print(f"  Score Loss Error: Unsupported pre_acts shape: {pre_acts.shape}")
            return torch.tensor(0.0, device=self.device, dtype=self.dtype)
    
        # Final validation
        if len(pre_acts.shape) != 2:
            print(f"  Score Loss Error: Final shape not 2D: {pre_acts.shape}")
            return torch.tensor(0.0, device=self.device, dtype=self.dtype)
    
        batch_size, num_latents = pre_acts.shape
    
        # CRITICAL: Ensure batch size matches labels
        if batch_size != len(object_labels) or batch_size != len(style_labels):
            print(f"  Score Loss Error: Batch size mismatch - pre_acts: {batch_size}, obj_labels: {len(object_labels)}, style_labels: {len(style_labels)}")
            return torch.tensor(0.0, device=self.device, dtype=self.dtype)
    
        # Ensure non-negative activations and check for NaN/Inf
        activations = torch.relu(pre_acts)
        if torch.isnan(activations).any() or torch.isinf(activations).any():
            print(f"  Score Loss Error: NaN/Inf in activations after ReLU")
            return torch.tensor(0.0, device=self.device, dtype=self.dtype)
    
        # Collect valid concepts and group by latent assignment
        latent_to_concepts = {}
        valid_concepts_found = 0
        
        # Process objects
        for obj_label in set(object_labels):
            if obj_label in object_to_latent:
                assigned_latent_idx = object_to_latent[obj_label]
                
                # CRITICAL: Validate latent index
                if not (0 <= assigned_latent_idx < num_latents):
                    print(f"  Score Loss Warning: Invalid object latent {assigned_latent_idx} for '{obj_label}' (max: {num_latents-1})")
                    continue
                    
                concept_mask = torch.tensor([obj == obj_label for obj in object_labels], 
                                          device=self.device, dtype=torch.bool)
                pos_count = concept_mask.sum().item()
                
                # Skip concepts with no positive or all positive samples
                if pos_count == 0 or pos_count == batch_size:
                    continue
                    
                if assigned_latent_idx not in latent_to_concepts:
                    latent_to_concepts[assigned_latent_idx] = []
                latent_to_concepts[assigned_latent_idx].append(('object', obj_label, concept_mask))
                valid_concepts_found += 1
    
        # Process styles
        for style_label in set(style_labels):
            if style_label != "none" and style_label in style_to_latent:
                assigned_latent_idx = style_to_latent[style_label]
                
                # CRITICAL: Validate latent index
                if not (0 <= assigned_latent_idx < num_latents):
                    print(f"  Score Loss Warning: Invalid style latent {assigned_latent_idx} for '{style_label}' (max: {num_latents-1})")
                    continue
                    
                concept_mask = torch.tensor([style == style_label for style in style_labels], 
                                          device=self.device, dtype=torch.bool)
                pos_count = concept_mask.sum().item()
                ...
                if assigned_latent_idx not in latent_to_concepts:
                    latent_to_concepts[assigned_latent_idx] = []
                latent_to_concepts[assigned_latent_idx].append(('style', style_label, concept_mask))
                valid_concepts_found += 1
    
        if valid_concepts_found == 0:
            print(f"  Score Loss: No valid concepts found in batch")
            return torch.tensor(0.0, device=self.device, dtype=self.dtype)
    
        # Compute score loss for each latent's concepts
        total_score_loss = 0.0
        num_computed_scores = 0
        
        for assigned_latent_idx, concept_list in latent_to_concepts.items():
            try:
                # Combine masks for concepts assigned to the same latent
                combined_pos_mask = torch.zeros(batch_size, device=self.device, dtype=torch.bool)
                concept_names = []
                
                for concept_type, concept_name, concept_mask in concept_list:
                    combined_pos_mask |= concept_mask
                    concept_names.append(f"{concept_type}:{concept_name}")
                
                # Final validation of combined mask
                pos_samples = combined_pos_mask.sum().item()
                if pos_samples == 0 or pos_samples == batch_size:
                    continue
                    
                neg_mask = ~combined_pos_mask
                
                # Get activations for positive and negative samples
                pos_activations = activations[combined_pos_mask]  # [pos_samples, num_latents]
                neg_activations = activations[neg_mask]          # [neg_samples, num_latents]
                
                # Validate activation tensors
                if pos_activations.numel() == 0 or neg_activations.numel() == 0:
                    print(f"    Warning: Empty activation tensor for latent {assigned_latent_idx}")
                    continue
                
                # Compute normalization terms with safety checks
                pos_mean_activations = pos_activations.mean(dim=0)  # [num_latents]
                neg_mean_activations = neg_activations.mean(dim=0)  # [num_latents]
                
                sum_mu_pos = pos_mean_activations.sum() + delta
                sum_mu_neg = neg_mean_activations.sum() + delta
                
                # Validate normalization terms
                if torch.isnan(sum_mu_pos) or torch.isnan(sum_mu_neg) or sum_mu_pos <= delta or sum_mu_neg <= delta:
                    print(f"    Warning: Invalid normalization terms for latent {assigned_latent_idx}")
                    continue
                
                # Get mean activation for this specific latent
                mu_i_pos = pos_mean_activations[assigned_latent_idx]
                mu_i_neg = neg_mean_activations[assigned_latent_idx]
                
                # Compute score using the exact definition
                score = (mu_i_pos / sum_mu_pos) - (mu_i_neg / sum_mu_neg)
                
                # Validate score
                if torch.isnan(score) or torch.isinf(score):
                    print(f"    Warning: Invalid score for latent {assigned_latent_idx}")
                    continue
                
                # Maximize score by minimizing negative score
                if score > 0:
                    total_score_loss -= score  # Reward positive scores
                else:
                    total_score_loss += 2 * abs(score)  # Heavily penalize negative scores

                num_computed_scores += 1
                
                # Debug output with safety
                score_value = score.item() if not torch.isnan(score) else float('nan')
                # print(f"    Latent {assigned_latent_idx} ({', '.join(concept_names[:2])}{'...' if len(concept_names) > 2 else ''}): score = {score_value:.6f}")
                
            except Exception as e:
                print(f"    Error computing score for latent {assigned_latent_idx}: {e}")
                continue
            
        if num_computed_scores == 0:
            print(f"  Score Loss: No valid scores computed")
            return torch.tensor(0.0, device=self.device, dtype=self.dtype)
    
        # Average the loss over all valid scores
        final_loss = total_score_loss / num_computed_scores
        
        # Final validation
        if torch.isnan(final_loss) or torch.isinf(final_loss):
            print(f"  Score Loss Error: Final loss is NaN/Inf")
            return torch.tensor(0.0, device=self.device, dtype=self.dtype)
        
        print(f"  Score Loss: {final_loss.item():.6f} (from {num_computed_scores} valid concepts)")
        return final_loss
    ...
    def evaluate_losses(self, sae, hook_name, object_to_latent, style_to_latent, is_validation=False):
        ...
        Modified to use score loss instead of binary cross-entropy loss.
        """
        ...
        with torch.no_grad():
            for batch_idx, (activations, object_labels, style_labels) in enumerate(loader):
                ...
                try:
                    ...
                    score_loss = self.compute_score_loss(pre_acts, object_labels, style_labels, object_to_latent, style_to_latent)

                    # Combined loss: MSE + Score
                    total_loss = (
        ...
        if num_batches > 0:
            ...
            avg_total_loss = avg_recon_loss = avg_score_loss = 0.0
    ...
    def log_batch_balance_stats(self, object_labels, style_labels, object_to_latent, style_to_latent, batch_idx):
        """Log statistics about batch balance for monitoring."""
        ...
        # Collect concepts present in batch
        for obj in object_labels:
            if obj in object_to_latent:
                batch_concepts.add(obj)
        for style in style_labels:
            if style != "none" and style in style_to_latent:
                batch_concepts.add(style)
        
        # Calculate positive/negative ratios
        for concept in batch_concepts:
            pos_count = 0
            if concept in object_to_latent:
                pos_count += sum(1 for obj in object_labels if obj == concept)
            if concept in style_to_latent:
                pos_count += sum(1 for style in style_labels if style == concept)
            
            neg_count = len(object_labels) - pos_count
            ratio = pos_count / len(object_labels)
            concept_stats[concept] = {"pos": pos_count, "neg": neg_count, "ratio": ratio}
        
        print(f"\nBatch {batch_idx} balance stats:")
        print(f"  Total concepts in batch: {len(batch_concepts)}")
        for concept, stats in concept_stats.items():
            print(f"  {concept}: {stats['pos']} pos, {stats['neg']} neg ({stats['ratio']:.1%})")
        
        if len(batch_concepts) == 0:
            print("  ⚠️ WARNING: No target concepts in this batch!")
    ...
    def train(self):
        ...
        for hook_name, sae in self.saes.items():
            ...
            if self.rank == 0:
                ...
                self.create_balanced_loaders_after_assignment(hook_name)
            ...
            if self.rank == 0 and self.start_epoch == 1:
                ...
                print("\n=== Initial Losses ===")
                print(f"  Training   - Total: {train_losses['total_loss']:.6f}, Recon: {train_losses['recon_loss']:.6f}, Score: {train_losses['score_loss']:.6f}")
                print(f"  Validation - Total: {val_losses['total_loss']:.6f}, Recon: {val_losses['recon_loss']:.6f}, Score: {val_losses['score_loss']:.6f}")
                ...
                if WANDB_AVAILABLE:
                    initial_metrics = {
                        ...
                        f"{hook_name}/initial/train/score_loss": train_losses['score_loss'],
                        f"{hook_name}/initial/val/total_loss": val_losses['total_loss'],
                        f"{hook_name}/initial/val/recon_loss": val_losses['recon_loss'],
                        f"{hook_name}/initial/val/score_loss": val_losses['score_loss'],
                    }
            ...
            for epoch in range(self.start_epoch, self.num_epochs + 1):
                ...
                for batch_idx, (activations, object_labels, style_labels) in enumerate(data_iter):
                    ...
                    if self.rank == 0:
                        self.log_batch_balance_stats(object_labels, style_labels, object_to_latent, style_to_latent, batch_idx)
                    ...
                    del recon_loss, score_loss, total_loss
                ...
                if self.world_size > 1:
                    ...
                    loss_tensor = torch.tensor(
                        [total_loss_sum, recon_loss_sum, score_loss_sum, num_batches],
                    ...
                    score_loss_sum = loss_tensor[2].item()
                ...
                if self.rank == 0:
                    ...
                    print(f"  Score Loss: {avg_score_loss:.6f}")
                    ...
                    if WANDB_AVAILABLE:
                        ...
                        metrics = {
                            ...
                            f"{hook_name}/train/score_loss": train_losses['score_loss'],
                            f"{hook_name}/val/total_loss": val_losses['total_loss'],
                            f"{hook_name}/val/recon_loss": val_losses['recon_loss'],
                            f"{hook_name}/val/score_loss": val_losses['score_loss'],
                        ...
                        self.log_summary_metrics(epoch, hook_name, train_losses, val_losses, train_success_rate, val_success_rate)
                        metrics.update({
...
def main():
    ...
    # Required parameters
    parser.add_argument("--score_weight", type=float, default=1.0, help="Weight for score loss")
    parser.add_argument(
        "--checkpoint_path", 
        type=str, 
        required=True, 
