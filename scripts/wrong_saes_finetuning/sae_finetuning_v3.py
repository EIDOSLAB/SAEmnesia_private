...
class SAEConceptLatentOptimizer:
    ...
    def compute_binary_cross_entropy_loss(self, pre_acts, object_labels, style_labels, object_to_latent, style_to_latent):
        ...
        # Shape: [batch_size, num_latents] - binary targets indicating which latents should be active
        ...
        if valid_mask.sum() == 0:
            print(f"  BCE Loss: No valid target positions")
            return torch.tensor(0.0, device=self.device, dtype=self.dtype)
        
        # Average loss only over valid target positions
        final_loss = bce_loss[valid_mask].mean()
        
        print(f"  BCE Loss: {final_loss.item():.6f} (from {valid_samples} samples, {valid_mask.sum().item()} target latents)")
        return final_loss
