...
class SAEConceptLatentOptimizer:
    ...
    def load_scores_data(self):
        ...
        # Replace the existing FileNotFoundError raises with:
    ...
    def compute_cross_entropy_loss(self, pre_acts, object_labels, style_labels, object_to_latent, style_to_latent, original_batch_size=None):
        """
        Enhanced cross-entropy loss that encourages both object and style latents to be active.
        ...
        print(f"  CE Loss: {final_loss.item():.6f} (from {valid_samples} samples, {valid_mask.sum().item()} target latents)")
        return final_loss
