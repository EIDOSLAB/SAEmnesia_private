...
def load_datasets_from_category_dirs(base_dirs, hookpoint, dtype=torch.float32):
    """
    Load datasets from concept directories with object labels only.
    """
    datasets = []
    ...
    # Print object distribution summary
...
class SAEConceptLatentOptimizer:
    ...
    This optimizer:
    1. Loads raw activations for different concepts with object labels
        ...
        try:
            ...
            if training_state_path.exists():
                try:
                    ...
                    if 'object_to_latent' in training_state:
                        self.object_to_latent[hook_name] = training_state['object_to_latent']
                        print(f"  ✅ Loaded object assignments: {len(self.object_to_latent[hook_name])} objects")
                    if 'style_to_latent' in training_state:
                        self.style_to_latent[hook_name] = training_state['style_to_latent']
                        print(f"  ✅ Loaded style assignments: {len(self.style_to_latent[hook_name])} styles")
                except Exception as e:
    ...
    def save_best_model(self, sae, hook_name, epoch, optimizer=None):
        ...
        try:
            ...
            training_state = {
                ...
                'object_to_latent': self.object_to_latent.get(hook_name, {}),  
                'style_to_latent': self.style_to_latent.get(hook_name, {}),    
            }
    ...
    def _create_data_loaders(self, dataset_dict):
        """DataLoader creation with object labels only."""
        ...
        def object_label_collate_fn(batch):
            """Collate function that handles object labels only."""
            ...
            return activations, object_labels
        ...
        self.train_loader = DataLoader(
            ...
            num_workers=2, pin_memory=True, collate_fn=object_label_collate_fn
        )
    ...
    def _assign_concepts_randomly(self, hook_name):
        ...
        # Get unique objects from a batch
        sample_batch = next(iter(self.train_loader))
        _, object_labels = sample_batch
        unique_objects = set(object_labels)

        print(f"Random assignment for {len(unique_objects)} objects")
        ...
        # Assign objects
        for i, obj in enumerate(sorted(unique_objects)):
            if i < len(available_latents):
                assigned_latent = available_latents[i]
        ...
        print(f"  Latents used: {min(len(unique_objects), model_num_latents)}/{model_num_latents}")

        return object_to_latent, {}  # Return empty dict for consistency

    def _assign_concepts_from_scores(self, hook_name):
        """Original score-based assignment logic."""
        if self.object_scores_data is None:
            raise RuntimeError("Object scores data not loaded.")
        ...
        if concepts_without_scores:
            ...
            if len(unused_latents) >= len(concepts_without_scores):
                ...
                for i, (concept_name, concept_type) in enumerate(concepts_without_scores):
                    ...
                    if concept_name not in object_to_latent:
                        object_to_latent[concept_name] = latent_idx
        ...
        total_assigned = assigned_objects 
        unique_latents_used = len(set(list(object_to_latent.values())))
    ...
    def assign_concepts_to_latents_from_scores(self, hook_name):
        # CHECK IF ALREADY LOADED FROM CHECKPOINT
        if hook_name in self.object_to_latent and self.object_to_latent[hook_name]:
            print(f"Using loaded assignments from checkpoint for {hook_name}")
            print(f"  Objects: {len(self.object_to_latent[hook_name])}")
            print(f"  Styles: {len(self.style_to_latent.get(hook_name, {}))}")
            return self.object_to_latent[hook_name], self.style_to_latent.get(hook_name, {})
        
        # OTHERWISE, CREATE NEW ASSIGNMENTS
    ...
    def print_epoch_summary(self, epoch, hook_name, train_losses, val_losses, 
                        train_distributions, val_distributions, object_to_latent):
        """
        ...
        combined_concept_to_latent = {**object_to_latent}
        ...
        for concept in sorted(all_concepts):
            concept_type = "object" if concept in object_to_latent else "unknown"
    ...
    def print_initial_concept_assignments(self, object_to_latent, hook_name):
        """
        ...
        print(f"Total concepts: {len(object_to_latent)}")
        print(f"="*70 + "\n")

    def print_latent_distribution_summary(self, distributions, object_to_latent, epoch=None, is_validation=False):
        """
    ...
    def get_latent_distribution_statistics(self, sae, data_loader, object_to_latent):
        ...
        with torch.no_grad():
            for batch_idx, batch_data in enumerate(data_loader):
                ...
                activations, object_labels= batch_data
                # Combine all concepts for analysis
                all_concepts = object_labels  
                ...
                try:
                    ...
                    for i, obj_concept in enumerate(object_labels):
                        # Object concept
    ...
    def initialize_saes(self):
        ...
        # Infer hook name from checkpoint path
        ...
        if (self.checkpoint_path / "cfg.json").exists() and (self.checkpoint_path / "sae.safetensors").exists():
            ...
            try:
                ...
                # If loading fails and from_scratch is True, create from scratch
                if self.from_scratch:
                    print(f"Creating SAE from scratch for {hook_name} due to load failure")
                    self._create_sae_from_scratch(hook_name)
        
        # If checkpoint doesn't exist but from_scratch is True, create it
        elif self.from_scratch:
            print(f"No checkpoint found, creating SAE from scratch for {hook_name}")
            self._create_sae_from_scratch(hook_name)
            ...
            # Handle subdirectories (if checkpoint path is a parent directory)
        ...
        # Final check: if still no SAEs and from_scratch is True, create one with path name
        if not self.saes and self.from_scratch:
            hook_name = self.checkpoint_path.name
            print(f"Final fallback: Creating SAE from scratch for {hook_name}")
            self._create_sae_from_scratch(hook_name)
    ...
    def initialize_wandb(self):
        ...
        if WANDB_AVAILABLE:
            ...
            run_name = f"sae_object_concept_optimization_{timestamp}"
            ...
            wandb.init(
                project="sae_object_concept_latent_optimizer",
                name=run_name,
                config=config,
                dir=wandb_dir
            )
    ...
    def compute_cross_entropy_loss(self, pre_acts, object_labels, object_to_latent, original_batch_size=None):
        """
        Simplified cross-entropy loss: Only uses object labels, no orthogonality constraint.
        """
        ...
        # Create target tensor for all samples - ONLY OBJECTS (no styles)
        target_mask = torch.zeros(batch_size, num_latents, device=self.device, dtype=torch.float32)
        
        valid_samples = 0
        
        # Only use object labels (ignore style_labels completely)
        for i, object_concept in enumerate(object_labels):
        ...
        # Return only the BCE loss (no orthogonality constraint)
        return ce_loss
    ...
    def evaluate_losses(self, sae, hook_name, object_to_latent, is_validation=False):
        """
        ...
        # Evaluate
        with torch.no_grad():
            for batch_idx, (activations, object_labels) in enumerate(loader):
                ...
                try:
                    ...
                    ce_loss = self.compute_cross_entropy_loss(pre_acts, object_labels, object_to_latent)
    ...
    def train(self):
        ...
        Enhanced with objectconcept assignment, resume functionality and early stopping.
        """
        ...
        for hook_name, sae in self.saes.items():
            ...
            if self.rank == 0:
                object_to_latent, _ = self.assign_concepts_to_latents_from_scores(hook_name)
                self.object_to_latent[hook_name] = object_to_latent
                self.print_initial_concept_assignments(object_to_latent, hook_name)
            ...
            if self.rank == 0 and self.start_epoch == 1:
                train_losses = self.evaluate_losses(sae, hook_name, self.object_to_latent[hook_name], is_validation=False)
                val_losses = self.evaluate_losses(sae, hook_name, self.object_to_latent[hook_name], is_validation=True)
            ...
            for epoch in range(self.start_epoch, self.num_epochs + 1):
                ...
                # Process batches
                for batch_idx, (activations, object_labels) in enumerate(data_iter):
                    if batch_idx % 10 == 0:
                ...
                if self.rank == 0:
                    ...
                    train_losses = self.evaluate_losses(sae, hook_name, object_to_latent, is_validation=False)
                    val_losses = self.evaluate_losses(sae, hook_name, object_to_latent, is_validation=True)
...
def main():
    """
    Main entry point for the SAE Object Concept Latent Optimizer.
    """
    parser = argparse.ArgumentParser(description="Optimize SAE models to assign specific latents to object concepts.")
    ...
    # Save parameters
    parser.add_argument("--save_dir", type=str, default="sae-object-concept-optimized", help="Directory to save optimized models")
