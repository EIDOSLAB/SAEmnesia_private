...
class SAEConceptLatentOptimizer:
    ...
    def load_scores_data(self):
        ...
        if not self.scores_json_path.exists():
            ...
            try:
                ...
                print(f"✅ Loaded object scores data:")
                ...
                if self.from_scratch:
                    print(f"⚠️  Error loading object scores, but training from scratch - will use random assignment: {e}")
                    self.scores_data = None
                else:
                    raise RuntimeError(f"Error loading object scores JSON file: {e}")

        # Load style scores if provided
        if self.style_scores_json_path and self.style_scores_json_path.exists():
            ...
            try:
                ...
                print(f"✅ Loaded style scores data:")
                ...
                print(f"🎯 DUAL CONCEPT MODE ENABLED - will assign both objects and styles to latents")

            except Exception as e:
                print(f"⚠️  Warning: Error loading style scores JSON file: {e}")
                print(f"   Continuing in single concept mode...")
                self.style_scores_data = None
                self.dual_concept_mode = False
        elif self.style_scores_json_path:
            if self.from_scratch:
                print(f"⚠️  Style scores file not found, but training from scratch - continuing in single concept mode")
            else:
                print(f"⚠️  Style scores file specified but not found: {self.style_scores_json_path}")
            self.dual_concept_mode = False
        else:
            print(f"ℹ️  No style scores provided - using single concept mode")
            self.dual_concept_mode = False

    def _assign_concepts_randomly(self, hook_name, unique_objects, unique_styles, model_num_latents):
        """Random assignment when training from scratch without scores."""
        print("Training from scratch without scores - assigning concepts randomly...")

        # Collect all concepts (objects + non-"none" styles)
        all_concepts = []
        concept_types = {}

        for obj in unique_objects:
            all_concepts.append(obj)
            concept_types[obj] = 'object'

        for style in unique_styles:
            if style != "none":
                all_concepts.append(style)
                concept_types[style] = 'style'

        print(f"Assigning {len(all_concepts)} concepts randomly to {model_num_latents} latents")
        ...
        for i, concept in enumerate(all_concepts):
            if i < len(available_latents):
                ...
                # More concepts than latents - allow duplicates
                latent_idx = random.randint(0, model_num_latents - 1)

            if concept_types[concept] == 'object':
                object_to_latent[concept] = latent_idx
            else:
                style_to_latent[concept] = latent_idx

            print(f"  Random {concept_types[concept]} '{concept}' → latent {latent_idx}")
    ...
    def assign_concepts_to_latents_from_scores(self, hook_name):
        ...
        Falls back to random assignment when training from scratch without scores.
        """
        ...
        # Check if we're training from scratch without scores
        if self.from_scratch and (self.scores_data is None or (self.dual_concept_mode and self.style_scores_data is None)):
            print("🎲 Training from scratch without sufficient scores - using random assignment")
            return self._assign_concepts_randomly(hook_name, unique_objects, unique_styles, model_num_latents)

        # Normal score-based assignment
    ...
    def print_initial_concept_assignments(self, object_to_latent, style_to_latent, hook_name):
        ...
        for concept, latent_idx in sorted(object_to_latent.items()):
            ...
            score = "N/A"
            if not self.from_scratch and object_scores:
        ...
        for concept, latent_idx in sorted(style_to_latent.items()):
            # Get the original score for this assignment
            score = "N/A"
            if not self.from_scratch and style_scores:
    ...
    def compute_binary_cross_entropy_loss(self, pre_acts, object_labels, style_labels, object_to_latent, style_to_latent):
        """
        Enhanced binary cross-entropy loss that encourages both object and style latents to be active.
        ...
        Args:
            pre_acts: Pre-activations from SAE encoder [batch_size, num_latents]
        ...
        Returns:
            Binary cross-entropy loss computed only on assigned latents
        """
        # print(f"  BCE Loss Debug: pre_acts shape = {pre_acts.shape}")
        ...
        if len(all_assigned_latents) == 0:
            print(f"  BCE Loss: No assigned latents found")
            return torch.tensor(0.0, device=self.device, dtype=self.dtype)
        ...
        if valid_samples == 0:
            print(f"  BCE Loss: No valid targets found")
            return torch.tensor(0.0, device=self.device, dtype=self.dtype)
    
        # Use binary cross-entropy with logits (more stable and autocast-safe)
        # This applies sigmoid internally and is numerically more stable
        ...
        if loss_mask.sum() == 0:
            print(f"  BCE Loss: No assigned latent positions")
            return torch.tensor(0.0, device=self.device, dtype=self.dtype)
        
        # Average loss only over assigned latent positions
        final_loss = bce_loss[loss_mask].mean()
        
        print(f"  BCE Loss: {final_loss.item():.6f} (from {valid_samples} samples, {loss_mask.sum().item()} assigned latent positions)")
        return final_loss
        ...
        with torch.no_grad():
            for batch_idx, (activations, object_labels, style_labels) in enumerate(loader):
                ...
                try:
                    ...
                    ce_loss = self.compute_binary_cross_entropy_loss(pre_acts, object_labels, style_labels, object_to_latent, style_to_latent)
                ...
                for batch_idx, (activations, object_labels, style_labels) in enumerate(data_iter):
                    ...
                    if self.mixed_precision and torch.cuda.is_available() and not self.use_float16:
                        with torch.amp.autocast('cuda'):
                            ...
                            # Binary cross-entropy loss for concept-specific latents (BEFORE topk)
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Optimize SAE models to assign specific latents to concepts with optional dual object-style support.")
