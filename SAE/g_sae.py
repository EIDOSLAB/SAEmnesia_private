...
class Sae(nn.Module):
    """
    Sparse Autoencoder with Sigmoid(TopK(h)) activation.
    
    Architecture:
        SAE(x) = D(σ(E(x)))
        where:
        - E(x) = W_enc * x + b_enc = h  (pre-activations)
        - σ(h) = Sigmoid(TopK(h)) = f   (activated latents)
        - D(f) = W_dec * f + b_dec = x̂ (reconstruction)
    """
    def __init__(
        self,
        d_in: int,
    ...
    def pre_acts(self, x: Tensor) -> Tensor:
        """
        Compute pre-activations h = W_enc * x + b_enc.
        
        Note: Removes decoder bias from input as per Anthropic's approach.
        Returns the raw pre-activations WITHOUT any activation function applied.
        """
        # Remove decoder bias as per Anthropic
        sae_in = x.to(self.dtype) - self.b_dec
        out = self.encoder(sae_in)
        
        # Return raw pre-activations (no ReLU, no Sigmoid yet)
        return out

    def pre_acts_without_relu(self, x: Tensor) -> Tensor:
        """
        Compute pre-activations without any activation function.
        This is now the same as pre_acts() since we removed ReLU.
        """
        # Remove decoder bias as per Anthropic
        sae_in = x.to(self.dtype) - self.b_dec
        out = self.encoder(sae_in)
    ...
    def select_topk(self, latents: Tensor, k=None, batch_size=None) -> EncoderOutput:
        """
        Select the top-k latents and apply Sigmoid activation.
        
        This implements σ(h) = Sigmoid(TopK(h)) = f
        
        For non-batch TopK (the default):
        1. Select top-k values from pre-activations h
        2. Apply Sigmoid to these top-k values
        3. Set all other latents to 0
        
        Args:
            latents: Pre-activations h (raw encoder outputs)
            k: Number of top latents to select
            batch_size: Batch size (used for batch/sample TopK variants)
            
        Returns:
            EncoderOutput with:
                - top_acts: Sigmoid(TopK(h)) - activated top-k latents
                - top_indices: Indices of the top-k latents
        """
        if k is None:
            k = self.cfg.k
        ...
        if self.cfg.batch_topk:
            ...
            # Apply Sigmoid to top-k values
            top_acts_flatten = torch.sigmoid(top_acts_flatten)
            ...
            # Apply Sigmoid to top-k values
            top_acts_sample = torch.sigmoid(top_acts_sample)
        ...
        # PatchTopK (default): Select k latents per patch
        # Total activated latents in batch: bs*sample_size*k
        # This is the standard TopK approach you requested
        top_vals, top_indices = latents.topk(k, sorted=False)
        
        # Apply Sigmoid to the top-k values: σ(TopK(h))
        top_acts = torch.sigmoid(top_vals)
        
        return EncoderOutput(top_acts=top_acts, top_indices=top_indices)

    def encode(self, x: Tensor) -> EncoderOutput:
        """
        Encode the input and select the top-k latents with Sigmoid activation.
        
        This implements: f = σ(E(x)) = Sigmoid(TopK(W_enc * x + b_enc))
        """
        batch_size, sample_size, emb_size = x.shape

        x = x.reshape(batch_size * sample_size, emb_size)

        # Get pre-activations h = E(x)
        h = self.pre_acts(x)
        
        # Apply σ(h) = Sigmoid(TopK(h))
        return self.select_topk(h)

    def decode(self, top_acts: Tensor, top_indices: Tensor) -> Tensor:
        """
        Decode the activated latents back to input space.
        
        This implements: D(f) = W_dec * f + b_dec = x̂
        
        Args:
            top_acts: Activated top-k latents f = Sigmoid(TopK(h))
            top_indices: Indices of the top-k latents
    ...
    def forward(self, x: Tensor, dead_mask: Tensor | None = None) -> ForwardOutput:
        ...
        Implements: SAE(x) = D(σ(E(x))) = D(Sigmoid(TopK(W_enc * x + b_enc)))
        
        Args:
            x: Input tensor of shape [bs, sample_size, d_in]
            dead_mask: Optional mask for dead latents (for AuxK loss)
            
        Returns:
            ForwardOutput with reconstruction and various losses
        """
        # x shape: [bs, sample_size, d_in]
        batch_size, sample_size, emb_size = x.shape
        x, x_mean, x_std = self.preprocess_input(x)
        
        # Get pre-activations h = E(x) = W_enc * x + b_enc
        pre_acts = self.pre_acts(x)  # [bs * sample_size, num_latents]
        
        # Apply activation: f = σ(h) = Sigmoid(TopK(h))
        # Decode and compute residual: x̂ = D(f)
        ...
        if dead_mask is not None and (num_dead := int(dead_mask.sum())) > 0:
            ...
            # Top-k dead latents with Sigmoid activation
        ...
        # L0 loss: count non-zero latents after activation
        # Since we use TopK, this should always be k (or less if there are fewer latents)
        l0_loss = (top_acts > 0).float().sum(-1).mean()
